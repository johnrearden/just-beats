...
class TestReviewForm(TestCase):
    ...
    def setUpTestData(cls):
        cls.test_user = User.objects.create(
            username='form_tester', password='pass')
        drumloop = Drumloop.objects.create(
            creator=cls.test_user, name='form_tester_loop')
    ...
    def test_rating_must_be_greater_than_0(self):
        ...
        drumloop = Drumloop.objects.create(
            creator=user, name='form_tester_loop_1')
    ...
    def test_rating_must_be_less_or_equal_to_5(self):
        ...
        drumloop = Drumloop.objects.create(
            creator=user, name='form_tester_loop_2')
        test_form = ReviewForm({
            'reviewer': user,
            'drumloop': drumloop,
            'rating': 10,
    ...
    def test_comment_is_required(self):
        ...
        drumloop = Drumloop.objects.create(
            creator=user, name='form_tester_loop_3')
        form = ReviewForm({'rating': 3, 'reviewer': user,
                           'drumloop': drumloop})
    ...
    def test_drumloop_field_is_hidden(self):
        self.assertEqual(
            self.test_form.fields['drumloop']
                .widget.__class__.__name__, 'HiddenInput')

    def test_reviewer_field_is_hidden(self):
        self.assertEqual(
            self.test_form.fields['reviewer']
                .widget.__class__.__name__, 'HiddenInput')
